<div x-data="{ showModal: @entangle('showModal') }" x-show="showModal" x-trap.noscroll="showModal" x-on:keydown.escape.window="showModal = false" x-cloak class="flex justify-center fixed inset-0 px-4 py-6 md:py-6 z-50">
  ...
  <div class=" bg-white rounded-lg transform p-6 pb-4 z-50 w-full h-full min-h-[600px] md:min-w-min md:w-2/5">
    ...
    <form class="flex flex-col gap-2" wire:submit.prevent="submit">
      <p class="text-center font-bold text-2xl hidden md:inline-block">Nov proizvod</p>
      <!-- Authors -->
        ...
        <div class="flex flex-col justify-center items-center">
          ...
          <div class="">
            <div class="flex flex-row">
              <label for="hidden_button" class="underline cursor-pointer hover:text-yellow-400">Izmeni</label>
              <img wire:loading wire:target="image" src="{{URL('/images/util/loading.gif')}}" alt="loading indicator" class="h-8 w-8 ">
            </div>
            <input type="file" wire:model="image" id="hidden_button" hidden>
          </div>
          <!-- End action-->
          <img src="{{$image ? $image->temporaryUrl() : URL('/images/util/noimage_alt.jpg')}}" width="140" height="240" alt="nova fotografija" class="@error('image') border-red-500 @enderror">
        </div>
        <!-- End image -->
      </div>
      <!-- End information -->
      ...
      <div class="flex flex-col gap-1" x-data="{ count: 0 }" x-init="count = $refs.counted.value.length">
        <textarea class="border border-black resize-none h-44 md:h-60 @error('book.synopsis') border-red-500 @enderror" x-ref="counted" x-on:keyup="count = $refs.counted.value.length" name="text" cols="70" maxlength="1024" placeholder="Unesite opis knjige" wire:model.defer="book.synopsis"></textarea>
        <!-- Char count -->
